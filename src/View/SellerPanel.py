...
class SellerPanel(scrolled.ScrolledPanel):
    ...
    def setupDateInput(self):
        ...
        maxWorkDaysText = wx.StaticText(self, label=u'最大连续休息天数')
        optionSizer.Add(maxWorkDaysText, pos=(1, 2), flag=wx.EXPAND | wx.TOP | wx.LEFT, border=15)

        self.maxWorkDaysInput = wx.TextCtrl(self, value='3', style=wx.TE_PROCESS_ENTER)
        ...
        self.startDateInput = wx.TextCtrl(self, value=TimeUtil.getMonday(TimeUtil.getToday()), style=wx.TE_PROCESS_ENTER)
        ...
        self.endDateInput = wx.TextCtrl(self, value=TimeUtil.getSunday(TimeUtil.getFormatedDate(TimeUtil.getToday(), 27)), style=wx.TE_PROCESS_ENTER)
    ...
    def updateGrid1(self, rows):
        ...
        for rowNum in range(len(rows)):
            if TimeUtil.isWeekend(rows[rowNum][0]):
                self.grid1.SetCellBackgroundColour(rowNum, 0, 'green')
                self.grid1.SetCellBackgroundColour(rowNum, 1, 'green')
                self.grid1.SetCellBackgroundColour(rowNum, 2, 'green')
    ...
    def onSchedule(self, evt):
        ...
        if not self.checkInput():
            return
        s = Scheduler(range(len(self.workers)), self.workloadInput.GetValue(), self.maxWorkDaysInput.GetValue())
